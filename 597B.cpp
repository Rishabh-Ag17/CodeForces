...
int rec(int idx, int lasttimeindex, int n, vector<pair<int, int>>& arr, vector<vector<int>>& dp) {
    if (idx == n) {
        return 0;
    }
    if (dp[idx][lasttimeindex + 1] != -1) {
        return dp[idx][lasttimeindex + 1];
    }
    int pick = 0, notpick = 0;
    // pick if lasttime is less than curr first time
    if (lasttimeindex == -1 || arr[lasttimeindex].second < arr[idx].first) {
        pick = 1 + rec(idx + 1, idx, n, arr, dp);
    }
    notpick = rec(idx + 1, lasttimeindex, n, arr, dp);
    return dp[idx][lasttimeindex + 1] = max(pick, notpick);
}
