...
vector<vector<int>> generateAllFactors() {
    
   

    // Iterate over all numbers from 1 to N
    for (int i = 1; i <= N; ++i) {
        // Iterate over multiples of the current number
        for (int j = i; j <= N; j += i) {
            factors[j].push_back(i); // Add 'i' as a factor of 'j'
        }
    }

    return factors;
}
bool isValidK(const string& s, int k) {
    int n = s.length();
    int diffCount = 0;
    ...
    for (int i = 0; i < n; ++i) {
        if (s[i] != s[i % k]) {
            if (diffCount == 1) {
                return false; // More than one different character
            }
            diffCount++;
        }
    }

    // If less than or equal to one different character found, k is valid
    return true;
}
bool isValidK2(const string& s, int k) {
    int n = s.length();
    int diffCount = 0;
...
signed main() {
    ...
    while (t--) {
        ...
        for(auto it:factors[n]){
            if(isValidK(s,it) || isValidK2(s,it)){
                cout<<it<<endl;
                break;
            }
        }
